...
2. Inspect only `<bundle_dir>/.aoa-skill-source.json`. If present, require
   `aoa_skill_source_receipt_v1`, bundle `titan-memory-loom`, owner `aoa-sdk`,
   valid absolute `owner_root`, safe `source_path`, and matching owner
   `SKILL.md`. A malformed present handle is terminal.
