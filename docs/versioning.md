...
requirements, owner-contour interpretation, guarded pruning, plan snapshot scope,
...
the compiler invent or grant either approval family. Exact capability-graph
IDs now bind directly; legacy aliases retain the same resolved meaning through
the pinned migration ledger, so this owner-source correction does not alter
the v3 plan semantics.
