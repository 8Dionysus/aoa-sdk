...
- the Codex agent must then apply the checkpoint skill protocol and run `aoa checkpoint review-note` to add the stronger semantic layer: real intermediate findings, candidate notes, stats hints, mechanic hints, closeout questions, and evidence refs in the note; that review now automatically inherits the matching auto-observation's provenance refs and checkpoint-phase skill context
- while any checkpoint commit still has `agent_review=pending`, next-step guidance must point to the missing `review-note` rather than directly to reviewed closeout
