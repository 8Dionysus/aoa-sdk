...
DECISION_PREFIX = "AOA-SDK-D"
DECISION_RE = re.compile(r"^AOA-SDK-D-(\d{4})-[a-z0-9][a-z0-9-]*\.md$")
DECISION_ID_RE = re.compile(r"^AOA-SDK-D-\d{4}$")
...
INDEX_PATHS = (
    ...
    INDEX_DIR / "by-sdk-facet.md",
    INDEX_DIR / "by-mechanic.md",
...
def collect_decision_records(repo_root: Path = REPO_ROOT) -> tuple[list[DecisionRecord], list[tuple[str, str]]]:
    ...
    for path in sorted(decision_root.glob(f"{DECISION_PREFIX}-*.md")):
        ...
        if not match:
            issues.append((rel, "filename must match AOA-SDK-D-####-slug.md"))
            continue
        ...
        sdk_facets = _metadata_value(metadata, "SDK facets", rel, issues)
        mechanic_parents = _metadata_value(metadata, "Mechanic parents", rel, issues)
        ...
        if decision_id and not DECISION_ID_RE.match(decision_id):
            issues.append((rel, "Decision ID must match AOA-SDK-D-####"))
        if original_date:
            try:
        ...
        if decision_id and original_date and title:
            records.append(
                DecisionRecord(
                    ...
                    sdk_facets=_split_values(sdk_facets),
                    mechanic_parents=_split_values(mechanic_parents),
...
def validate_index_contract(repo_root: Path = REPO_ROOT) -> list[tuple[str, str]]:
    ...
    text = path.read_text(encoding="utf-8")
    required = [
        "schema_version: aoa_sdk_decision_index_contract_v1",
        "authority: docs/decisions/AGENTS.md",
        "source_records: docs/decisions/AOA-SDK-D-*.md",
...
def _table(records: list[DecisionRecord], *, columns: tuple[str, ...]) -> str:
    ...
    for record in records:
        ...
        for column in columns:
            ...
            elif column == "SDK facets":
                values.append(", ".join(record.sdk_facets))
            elif column == "Mechanic parents":
                values.append(", ".join(record.mechanic_parents))
...
def _grouped_index(
    ...
    *,
    ...
    for group_name in sorted(groups, key=str.lower):
        lines.extend(["\n", f"## {group_name}\n", "\n"])
        lines.append(_table(sorted(groups[group_name], key=lambda item: item.decision_id), columns=columns))
    return "".join(lines)
...
def render_index_files(records: list[DecisionRecord]) -> dict[Path, str]:
    ...
    return {
        ...
        INDEX_DIR / "by-number.md": (
            ...
            + _table(
                by_number,
                columns=("Decision", "Date", "Surface classes", "SDK facets", "Mechanic parents", "Guard families", "Posture"),
            )
        ),
        INDEX_DIR / "by-date.md": (
            "# Decisions By Date\n\n"
        ...
        INDEX_DIR / "by-surface.md": _grouped_index(
            ...
            columns=("Decision", "Date", "SDK facets", "Mechanic parents", "Guard families", "Posture"),
        ),
        INDEX_DIR / "by-sdk-facet.md": _grouped_index(
            "Decisions By SDK Facet",
            "Grouped by `SDK facets` metadata.",
            _group_by(records, "sdk_facets"),
            columns=("Decision", "Date", "Surface classes", "Mechanic parents", "Guard families", "Posture"),
        ),
        INDEX_DIR / "by-mechanic.md": _grouped_index(
            "Decisions By Mechanic Parent",
            "Grouped by `Mechanic parents` metadata.",
            _group_by(records, "mechanic_parents"),
            columns=("Decision", "Date", "Surface classes", "SDK facets", "Guard families", "Posture"),
        ),
        INDEX_DIR / "by-guard.md": _grouped_index(
            ...
            columns=("Decision", "Date", "Surface classes", "SDK facets", "Mechanic parents", "Posture"),
        ),
    }
