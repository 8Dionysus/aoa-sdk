...
def load(args):
    p = Path(args.session)
    return TitanAppServerBridgeSession.load(p), p


def iter_messages(args):
    if getattr(args, "message", None):
        yield json.loads(args.message)
...
def cmd_emit(a):
    ...
    if a.kind == "initialize":
        msgs = [b.initialize(), b.initialized()]
    elif a.kind == "thread-start":
        msgs = [
            b.thread_start(
                model=a.model,
                cwd=a.workspace,
    ...
    elif a.kind == "turn-start":
        msgs = [
            b.turn_start(
                thread_id=a.thread_id,
                text=a.text or "Render Titan bridge status.",
                cwd=a.workspace,
            )
        ]
    elif a.kind == "turn-steer":
        msgs = [
            b.turn_steer(thread_id=a.thread_id, text=a.text or "Steer Titan bridge.")
        ]
    else:
        raise SystemExit("unknown emit kind")
    for m in msgs:
...
def cmd_metrics(a):
    s, _ = load(a)
    print(json.dumps(s.metrics(), indent=2, ensure_ascii=False))
    return 0
...
def parser():
    p = argparse.ArgumentParser(description="Titan app-server bridge seed CLI")
    ...
    q = sub.add_parser("emit")
    q.add_argument(
        "kind", choices=["initialize", "thread-start", "turn-start", "turn-steer"]
    )
    q.add_argument("--workspace")
    q.add_argument("--model", default="gpt-5.4")
    q.add_argument("--approval-policy", default="unlessTrusted")
    q.add_argument(
        "--sandbox", default="workspaceWrite", choices=["readOnly", "workspaceWrite"]
    )
    q.add_argument("--thread-id")
    q.add_argument("--text")
    q.set_defaults(func=cmd_emit)
    ...
    q.add_argument(
        "--titan",
        required=True,
        choices=["Atlas", "Sentinel", "Mneme", "Forge", "Delta"],
    )
    ...
    q.add_argument("--session", required=True)
    q.set_defaults(func=cmd_metrics)
