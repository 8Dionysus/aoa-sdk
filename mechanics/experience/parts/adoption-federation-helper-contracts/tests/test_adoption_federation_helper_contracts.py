...
ADOPTION_FEDERATION_STEMS = (
    "sdk_adoption_dashboard_call",
    "sdk_adoption_request_call",
    "sdk_adoption_rollback_call",
    "sdk_cross_repo_signal_call",
    "sdk_harvest_run_call",
    "sdk_kag_dossier_call",
    "sdk_pattern_registry_call",
    "sdk_shadow_run_call",
    "sdk_tos_dossier_call",
)
...
ENUM_ESCAPE_VALUE = "__adoption_federation_not_allowed__"


def load_contract(stem: str) -> tuple[dict[str, object], dict[str, object]]:
    schema_path = PART_ROOT / "schemas" / f"{stem}_v1.json"
...
class AdoptionFederationHelperContractTests(unittest.TestCase):
    ...
    def test_adoption_federation_examples_match_schemas(self) -> None:
        missing_pairs: list[str] = []
        for stem in ADOPTION_FEDERATION_STEMS:
            schema_path = PART_ROOT / "schemas" / f"{stem}_v1.json"
            example_path = PART_ROOT / "examples" / f"{stem}.example.json"
        ...
        self.assertFalse(
            missing_pairs,
            "missing adoption/federation contract pair(s): " + ", ".join(missing_pairs),
        )

        self.assertTrue(ADOPTION_FEDERATION_STEMS)
        for stem in ADOPTION_FEDERATION_STEMS:
    ...
    def test_adoption_federation_schemas_reject_escape_hatches(self) -> None:
        self.assertTrue(ADOPTION_FEDERATION_STEMS)
        for stem in ADOPTION_FEDERATION_STEMS:
            with self.subTest(stem=stem):
                schema, example = load_contract(stem)
    ...
    def test_adoption_federation_schemas_reject_guardrail_boolean_inversions(self) -> None:
        for stem in ADOPTION_FEDERATION_STEMS:
            schema, example = load_contract(stem)
            payload = example.get("payload")
            if not isinstance(payload, dict):
    ...
    def test_adoption_federation_schemas_reject_invalid_numeric_ranges(self) -> None:
        for stem in ADOPTION_FEDERATION_STEMS:
            schema, example = load_contract(stem)
            payload = example.get("payload")
            if not isinstance(payload, dict):
    ...
    def test_adoption_federation_schemas_reject_non_string_array_items(self) -> None:
        exercised = 0
        for stem in ADOPTION_FEDERATION_STEMS:
            schema, example = load_contract(stem)
        ...
        self.assertGreater(exercised, 0, "no adoption/federation array fields were exercised")

    def test_adoption_federation_schemas_reject_enum_escape_values(self) -> None:
        exercised = 0
        for stem in ADOPTION_FEDERATION_STEMS:
            schema, example = load_contract(stem)
        ...
        self.assertGreater(exercised, 0, "no adoption/federation enum fields were exercised")
