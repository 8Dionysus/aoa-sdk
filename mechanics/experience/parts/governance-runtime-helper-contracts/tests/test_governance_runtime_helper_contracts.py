...
class GovernanceRuntimeHelperContractTests(unittest.TestCase):
    ...
    def test_governance_runtime_examples_match_schemas(self) -> None:
        ...
        self.assertFalse(
            missing_pairs,
            "missing governance/runtime contract pair(s): " + ", ".join(missing_pairs),
        )
    ...
    def test_governance_runtime_schemas_reject_empty_decision_refs(self) -> None:
        exercised = 0
        for stem, schema_file in GOVERNANCE_RUNTIME_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
        ...
        self.assertGreater(exercised, 0, "no governance/runtime decision_refs arrays were exercised")
