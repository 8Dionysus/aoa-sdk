...
ESCAPE_VALUE = "__office_release_train_not_allowed__"

OFFICE_RELEASE_TRAIN_CONTRACTS = (
    ('sdk_console_api_call_v1', 'sdk_console_api_call_v1.json'),
    ('sdk_handoff_graph_api_call_v1', 'sdk_handoff_graph_api_call_v1.json'),
    ('sdk_install_api_call_v1', 'sdk_install_api_call_v1.json'),
    ('sdk_office_registry_api_call_v1', 'sdk_office_registry_api_call_v1.json'),
    ('sdk_office_train_api_call_v1', 'sdk_office_train_api_call_v1.json'),
    ('sdk_release_api_call_v1', 'sdk_release_api_call_v1.json'),
)
...
class OfficeReleaseTrainHelperContractTests(unittest.TestCase):
    ...
    def test_office_release_train_examples_match_schemas(self) -> None:
        self.assertTrue(OFFICE_RELEASE_TRAIN_CONTRACTS)
        missing_pairs: list[str] = []
        for stem, schema_file in OFFICE_RELEASE_TRAIN_CONTRACTS:
            schema_path = PART_ROOT / "schemas" / schema_file
            example_path = PART_ROOT / "examples" / f"{stem}.example.json"
            if not schema_path.exists():
                missing_pairs.append(f"{example_path.relative_to(PART_ROOT)} -> {schema_path.relative_to(PART_ROOT)}")
            if not example_path.exists():
                missing_pairs.append(f"{schema_path.relative_to(PART_ROOT)} -> {example_path.relative_to(PART_ROOT)}")
        self.assertFalse(
            missing_pairs,
            "missing office/release-train contract pair(s): " + ", ".join(missing_pairs),
        )
    ...
    def test_office_release_train_schemas_reject_unknown_fields(self) -> None:
        exercised = 0
        for stem, schema_file in OFFICE_RELEASE_TRAIN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_office_release_train_schemas_reject_wrong_types_for_every_field(self) -> None:
        exercised = 0
        for stem, schema_file in OFFICE_RELEASE_TRAIN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_office_release_train_schemas_reject_missing_required_fields(self) -> None:
        exercised = 0
        for stem, schema_file in OFFICE_RELEASE_TRAIN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_office_release_train_schemas_reject_bad_array_items(self) -> None:
        exercised = 0
        for stem, schema_file in OFFICE_RELEASE_TRAIN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_office_release_train_schemas_reject_empty_strings(self) -> None:
        exercised = 0
        for stem, schema_file in OFFICE_RELEASE_TRAIN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_office_release_train_schemas_reject_const_escapes(self) -> None:
        exercised = 0
        for stem, schema_file in OFFICE_RELEASE_TRAIN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_office_release_train_schemas_reject_enum_escapes(self) -> None:
        exercised = 0
        for stem, schema_file in OFFICE_RELEASE_TRAIN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
    ...
    def test_office_release_train_schemas_reject_numeric_bound_escapes(self) -> None:
        for stem, schema_file in OFFICE_RELEASE_TRAIN_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
            for path, value in walk_values(example):
