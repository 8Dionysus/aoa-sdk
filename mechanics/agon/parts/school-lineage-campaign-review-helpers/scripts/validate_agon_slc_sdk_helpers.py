...
ENTRY_SCHEMA = ROOT / 'schemas' / 'agon-slc-sdk-helper.schema.json'
REGISTRY_SCHEMA = ROOT / 'schemas' / 'agon-slc-sdk-helper-registry.schema.json'
BUILDER = ROOT / 'scripts' / 'build_agon_slc_sdk_helpers.py'
...
def load_builder():
    spec = importlib.util.spec_from_file_location('_agon_slc_builder', BUILDER)
...
def validate_item(item: dict[str, Any]) -> str | None:
    ...
    if item.get('surface_role') != 'school_lineage_campaign_review_helpers':
        return f'{key} surface_role must be school_lineage_campaign_review_helpers'
...
def validate() -> int:
    ...
    if source.get('surface_role') != 'school_lineage_campaign_review_helpers':
        return fail('source surface_role must be school_lineage_campaign_review_helpers')
    if source.get('review_domain') != 'schools_lineages_campaigns':
        return fail('source review_domain mismatch')
