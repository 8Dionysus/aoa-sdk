...
def test_slc_registry_shape():
    reg = load(OUT)
    assert reg['surface_role'] == 'school_lineage_campaign_review_helpers'
    assert reg['review_domain'] == 'schools_lineages_campaigns'
    ...
    for item in reg[ITEM_KEY]:
        assert item['surface_role'] == 'school_lineage_campaign_review_helpers'
