...
VALIDATOR = ROOT / 'scripts/validate_agon_sophian_sdk_helpers.py'
EXPECTED_COUNT = 7
ITEM_KEY = 'sophian_sdk_helper_candidates'

def test_generated_registry_shape():
    reg = json.loads(GENERATED.read_text(encoding='utf-8'))
    assert reg['surface_role'] == 'sophian_threshold_review_helpers'
    assert reg['review_domain'] == 'sophian_threshold'
