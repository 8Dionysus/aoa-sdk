...
ENTRY_SCHEMA = ROOT / 'schemas/agon-sophian-sdk-helper.schema.json'
REGISTRY_SCHEMA = ROOT / 'schemas/agon-sophian-sdk-helper-registry.schema.json'
BUILDER = ROOT / 'scripts/build_agon_sophian_sdk_helpers.py'
ITEM_KEY = 'sophian_sdk_helper_candidates'
REGISTRY_ID = 'agon.sophian_sdk_helpers.registry.v1'
SURFACE_ROLE = 'sophian_threshold_review_helpers'
REVIEW_DOMAIN = 'sophian_threshold'
...
def validate_item(item: dict[str, Any]) -> str | None:
    ...
    if item.get('surface_role') != SURFACE_ROLE:
        return f'{key} surface_role must be {SURFACE_ROLE}'
