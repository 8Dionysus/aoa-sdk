...
from build_agon_epistemic_sdk_helpers import (  # noqa: E402
    BuildError,
    ITEM_KEY,
    OUT,
    REGISTRY_ID,
...
EXPECTED_COUNT = 7
KEY_FIELD = "helper_id"
REQUIRED_FIELDS = ['candidate_outputs', 'forbidden_effects', 'helper_for', 'helper_id', 'live_protocol', 'owner_repo', 'runtime_effect', 'status']
...
ITEM_SCHEMA = ROOT / "schemas" / "agon-epistemic-sdk-helper.schema.json"
REGISTRY_SCHEMA = ROOT / "schemas" / "agon-epistemic-sdk-helper-registry.schema.json"
...
def validate_item(item: Any, item_validator: jsonschema.Draft202012Validator) -> str | None:
    ...
    if item.get("owner_repo") != "aoa-sdk":
        return f"{key} owner_repo must be aoa-sdk"
...
def validate() -> int:
    ...
    if data.get("surface_role") != SURFACE_ROLE:
        return fail(f"source surface_role must be {SURFACE_ROLE}")
