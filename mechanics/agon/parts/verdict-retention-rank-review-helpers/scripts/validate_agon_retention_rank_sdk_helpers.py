...
REGISTRY = ROOT / "generated/agon_retention_rank_sdk_helpers.min.json"
SCHEMA = ROOT / "schemas/agon-retention-rank-sdk-helper-registry.schema.json"
BUILDER = ROOT / "scripts/build_agon_retention_rank_sdk_helpers.py"
...
def expected_registry() -> dict:
    spec = importlib.util.spec_from_file_location("agon_retention_rank_sdk_builder", BUILDER)
...
def main() -> int:
    ...
    if data.get("live_protocol") is not EXPECTED_LIVE_PROTOCOL:
        raise SystemExit("retention/rank registries must remain non-live protocol surfaces")
