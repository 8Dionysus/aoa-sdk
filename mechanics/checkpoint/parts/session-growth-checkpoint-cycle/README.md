...
attaches read-only session-memory archive refs, and materializes reviewed
checkpoint evidence plus exact owner candidates. It also exposes lifecycle
audit and close/archive routing so old `current/` checkpoint scopes stop
pretending to be active work once review and materialization evidence prove their
...
- explicit `aoa checkpoint mark`, `append`, `after-commit`, and
  `review-note` calls
- host runtime metadata and session-local checkpoint ledgers
- aoa-session-memory session registry, manifest, index, and raw-block refs
- reviewed closeout context, reviewed artifact evidence, materialization
  receipts, and owner-candidate handoffs
...
- reviewed evidence bundles and owner-candidate handoffs with
...
Mutable capture, review, context, and materialization routes require an
explicit host runtime identity. The SDK never creates one and never falls back
to an unscoped mutable session merely to make the operation succeed.

## Next Route

After review, materialize surviving evidence and exact owner candidates, then
route real follow-through to the owner workflow, memory, proof, progression,
quest, or stats surface. After reviewed evidence materialization, route the
checkpoint scope to lifecycle close/archive so `current/` remains active-now
or still-blocked review work. Materialization never proves that a capability or
owner mutation ran.
