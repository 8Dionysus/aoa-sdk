...
def test_closeout_enqueue_current_cli_can_emit_json(workspace_root: Path) -> None:
...
def test_closeout_build_manifest_cli_can_emit_json(workspace_root: Path) -> None:
    fixture = install_closeout_fixture(workspace_root)
    runner = CliRunner()
...
def test_closeout_submit_reviewed_cli_can_emit_json(workspace_root: Path) -> None:
    fixture = install_closeout_fixture(workspace_root)
    runner = CliRunner()
...
def test_closeout_submit_reviewed_cli_accepts_diagnosis_packet_receipts(
    workspace_root: Path,
) -> None:
    fixture = install_closeout_fixture(workspace_root)
    runner = CliRunner()
        ...
        json.dumps(
            {
                ...
                "evidence_refs": [
                    {"kind": "diagnosis_packet", "ref": str(fixture["reviewed_artifact_path"])}
                ],
                "payload": {
                    "route_ref": "route:test-closeout",
                    "skill_name": "aoa-session-self-diagnose",
                    ...
                    "probable_cause_hypotheses": [
                        "reviewed follow-through still needs explicit filtering"
                    ],
        ...
        json.dumps(
            {
                ...
                "evidence_refs": [
                    {"kind": "detail_receipt", "ref": str(diagnosis_receipt_path)}
                ],
                "payload": {
                    "kernel_id": "project-core-session-growth-v1",
                    "skill_name": "aoa-session-self-diagnose",
...
def test_closeout_submit_reviewed_allow_empty_cli_can_emit_json(
    workspace_root: Path,
) -> None:
    fixture = install_closeout_fixture(workspace_root)
    runner = CliRunner()
...
def test_closeout_run_cli_prints_kernel_next_brief(workspace_root: Path) -> None:
    fixture = install_closeout_fixture(workspace_root)
    runner = CliRunner()
...
def test_closeout_process_inbox_cli_prints_kernel_next_brief(
    workspace_root: Path,
) -> None:
...
def test_closeout_api_run_surfaces_workflow_follow_through_for_reanchor_progression(
    ...
    quest_receipt = {
        ...
        "payload": {
            ...
            "owner_repo": "aoa-playbooks",
            "next_surface": "quests/checkpoint/captured/test-closeout-reanchor-workflow-followup.md",
            "nearest_wrong_target": "promote_to_skill",
