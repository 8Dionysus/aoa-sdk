...
def test_playbooks_local_read_path(workspace_root: Path) -> None:
    ...
    assert registry_entry.recovery_mode == "rollback"
    assert "fallback_mode" not in registry_entry.model_dump()
    assert activation.memo_source_route_policy == "required"
    assert activation.recovery_mode == "rollback"
    assert "fallback_mode" not in activation.model_dump()
