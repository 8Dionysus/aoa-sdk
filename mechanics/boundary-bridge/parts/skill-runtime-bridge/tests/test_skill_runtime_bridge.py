...
def _load_install_closeout_fixture():
    ...
    fixture_module_path = (
        ...
        / "tests"
        / "test_reviewed_session_handoff_runner.py"
    )
    spec = spec_from_file_location("reviewed_session_handoff_runner_tests", fixture_module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"could not load closeout fixture module from {fixture_module_path}")
    module = module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.install_closeout_fixture
...
def test_detect_closeout_reuses_kernel_brief(workspace_root: Path, install_host_skills) -> None:
    ...
    install_closeout_fixture = _load_install_closeout_fixture()
...
def test_detect_demotes_activate_now_when_host_inventory_marks_skill_router_only(workspace_root: Path) -> None:
    ...
    assert report.must_confirm[0].host_availability.manual_equivalence_allowed is True
