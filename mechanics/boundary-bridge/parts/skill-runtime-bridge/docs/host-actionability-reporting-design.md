# Host Actionability Reporting Design
...
See [recommendation-actionability-gap.md](recommendation-actionability-gap.md)
for the recorded operating problem and temporary manual-equivalence rule.
...
- `router-only`
  ...
  but the operator must use manual equivalence or another route.
...
- set `manual_equivalence_allowed = true`
...
must_confirm:
  - aoa-sanitized-share [risk-ring / safety-and-mutation-gating / router-only]
    reason: required explicit risk gate for mutation_surface=public-share
    host: router-only via host-skill-list; manual equivalence allowed
```
...
| `must_confirm` | `router-only` | stay in `must_confirm`, mark manual equivalence |
| `must_confirm` | `unknown` | stay in `must_confirm`, annotate `unknown` |
| `suggest_next` | `host-executable` | stay in `suggest_next` |
| `suggest_next` | `router-only` | stay in `suggest_next`, mark manual equivalence |
...
- explicit report verdicts for "manual equivalence required"
