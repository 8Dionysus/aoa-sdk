# Recommendation Actionability Gap
...
### 3. Use explicit manual equivalence while recording the gap
...
supplied, use this operating rule:
...
- if the work continues, state that the route is following a manual equivalence
...
- the old hidden gap is now downgraded into an explicit manual-equivalence case
  when no install root is present
...
[host-actionability-reporting-design.md](host-actionability-reporting-design.md).
...
`aoa-skills` and without treating one session bridge as permanent doctrine.
