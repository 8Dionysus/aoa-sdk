...
The initial boundary keeps heuristics in Python code under `src/aoa_sdk/surfaces/`.
...
When intent text directly names an AoA layer, the initial boundary can surface a
...
- router-only `must_confirm` or router-only `suggest_next` items with
  `manual_equivalence_allowed=true` become `state=manual-equivalent` and
