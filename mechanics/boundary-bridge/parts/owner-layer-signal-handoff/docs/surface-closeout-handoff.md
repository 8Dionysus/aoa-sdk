...
The reviewed handoff boundary adds one reviewed-only handoff contract for surviving surface notes.
...
`aoa-session-progression-lift` remain legal targets in the schema, but this
handoff boundary does not auto-generate them without a later deterministic
signal.
