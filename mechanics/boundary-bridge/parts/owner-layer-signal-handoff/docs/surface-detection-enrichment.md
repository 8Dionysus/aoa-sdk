# AoA Surface Detection Enrichment Layer

Surface detection enrichment stays additive to the initial boundary in
`aoa-sdk`. It sharpens visibility; it does not create a new semantic owner.

## Purpose

The enrichment layer adds three bounded enrichments around the existing
`sdk.surfaces` flow:
