# Codex Projection Portability Boundary

This note clarifies how `aoa-sdk` should relate to the shared-root AoA Codex
projection surface when the live workspace root changes.
...
- regenerate the checked-in `8Dionysus/.codex/config.toml` and
  `8Dionysus/.codex/hooks.json` from the `8Dionysus` Codex projection
...
hidden owner of Codex deployment state.
