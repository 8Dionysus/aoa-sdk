...
def test_live_rollout_status_readout_example_validates_against_schema() -> None:
    schema = json.loads(
        (PART_ROOT / "schemas" / "live-rollout-status-snapshot.schema.json").read_text(
            encoding="utf-8"
        )
    )
    example = json.loads(
        (PART_ROOT / "examples" / "live-rollout-status-snapshot.example.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_live_rollout_status_readout_reads_external_rollout_artifacts(
    workspace_root: Path,
) -> None:
...
def test_live_rollout_status_readout_requires_live_rollout_artifacts(workspace_root: Path) -> None:
...
def test_live_rollout_status_readout_rejects_missing_live_refs(workspace_root: Path) -> None:
    rollout_root = workspace_root / ".codex" / "generated" / "rollout"
    write_json(
        rollout_root / "codex_plane_trust_state.current.json",
        {
...
def test_live_rollout_status_readout_rejects_malformed_timestamps(workspace_root: Path) -> None:
    rollout_root = workspace_root / ".codex" / "generated" / "rollout"
    write_json(
        rollout_root / "codex_plane_trust_state.current.json",
        {
