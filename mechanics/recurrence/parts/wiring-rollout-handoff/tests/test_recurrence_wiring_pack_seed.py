...
REPO_ROOT = next(parent for parent in TEST_PATH.parents if (parent / "src" / "aoa_sdk").is_dir())
SRC_ROOT = REPO_ROOT / "src" / "aoa_sdk"
...
def test_wiring_plan_ci_commands_use_repo_root_test_paths(tmp_path: Path) -> None:
    ...
    assert ci_snippet.commands[1].startswith(
        "pytest -q mechanics/recurrence/parts/component-manifest-gate/tests/test_recurrence_seed.py"
    )
