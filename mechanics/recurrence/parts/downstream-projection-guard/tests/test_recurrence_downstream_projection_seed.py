...
ROOT = next(parent for parent in TEST_PATH.parents if (parent / "src" / "aoa_sdk").is_dir())
PART_ROOT = TEST_PATH.parents[1]
...
def test_routing_projection_is_advisory_and_thin() -> None:
    ...
    schema = json.loads(
        (PART_ROOT / "schemas/recurrence-routing-projection.schema.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_stats_projection_no_input_path_keeps_schema_provenance() -> None:
    ...
    schema = json.loads(
        (PART_ROOT / "schemas/recurrence-stats-projection.schema.json").read_text(
            encoding="utf-8"
        )
    )
    Draft202012Validator(schema).validate(projection.model_dump(mode="json"))
...
def test_kag_projection_regrounds_without_claiming_canon() -> None:
    ...
    schema = json.loads(
        (PART_ROOT / "schemas/recurrence-kag-projection.schema.json").read_text(
            encoding="utf-8"
        )
    )
    Draft202012Validator(schema).validate(projection.model_dump(mode="json"))
